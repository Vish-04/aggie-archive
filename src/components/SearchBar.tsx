...
export default function SearchBar({onToggleClass, isCurrent}: SearchBarProps) {
	...
	return (
		<div ref={wRef} className="w-full my-4">
			<div className="flex items-center border border-[#AB97CC] rounded-[8px] px-4 py-3 bg-white"
			...
			{isOpen && (
				<div
					className="absolute left-12 md:left-20 lg:left-24 border-t-0 right-12 md:right-20 lg:right-24 bg-white border border-purpleBorderGray rounded-b-lg shadow-md z-10 p-6 max-h-[468px] overflow-y-auto w-1152px">
					{loading && <p className="text-gray-600">Loading...</p>}
					{!loading && classes.length === 0 && <p className="text-black">No courses found.</p>}
					{!loading &&
						classes.map((crs) => (
							<div key={crs.id}
									 className="py-6 p-[30px] mb-4 rounded-b-[10px] border-[#B0B0B0] rounded-t-[10px] w-full h-fit last:border-none bg-purpleGray shadow-md">
								{/* class name + checkbox svg*/}
								<div className="flex justify-between mb-1 items-center">
									<div className="flex flex-col gap-1">
										<p className="text-[24px] font-bold text-darkPurple">
											{crs.course_code}
										</p>
										<p className="hidden md:block text-sm md:text-[16px] font-[400] text-darkPurple">
											{crs.title}
										</p>
									</div>
									{/* checkmark button */}
									<button className="cursor-pointer" onClick={() => {
										...
										{isCurrent(crs.course_code) ? (
											<div className="flex items-center gap-2">
												<span className="">Added to <span
													className="text-brightPurple font-semibold">My Classes</span></span>
												<svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none"
														 className="mt-1">
													<path fillRule="evenodd" clipRule="evenodd"
																d="M12.5939 23.2579L12.5819 23.2599L12.5109 23.2949L12.4909 23.2989L12.4769 23.2949L12.4059 23.2589C12.3953 23.2563 12.3873 23.2583 12.3819 23.2649L12.3779 23.2749L12.3609 23.7029L12.3659 23.7229L12.3759 23.7359L12.4799 23.8099L12.4949 23.8139L12.5069 23.8099L12.6109 23.7359L12.6229 23.7199L12.6269 23.7029L12.6099 23.2759C12.6073 23.2653 12.6019 23.2593 12.5939 23.2579ZM12.8579 23.1449L12.8439 23.1469L12.6599 23.2399L12.6499 23.2499L12.6469 23.2609L12.6649 23.6909L12.6699 23.7029L12.6779 23.7109L12.8789 23.8029C12.8916 23.8063 12.9013 23.8036 12.9079 23.7949L12.9119 23.7809L12.8779 23.1669C12.8746 23.1543 12.8679 23.1469 12.8579 23.1449ZM12.1429 23.1469C12.1385 23.1443 12.1333 23.1434 12.1282 23.1445C12.1232 23.1456 12.1188 23.1487 12.1159 23.1529L12.1099 23.1669L12.0759 23.7809C12.0766 23.7929 12.0823 23.8009 12.0929 23.8049L12.1079 23.8029L12.3089 23.7099L12.3189 23.7019L12.3219 23.6909L12.3399 23.2609L12.3369 23.2489L12.3269 23.2389L12.1429 23.1469Z"
																fill="#483183"/>
													<path fillRule="evenodd" clipRule="evenodd"
											...
											<svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none"
													 className="mt-1">
												<path
													d="M19.5 20.625H4.5C4.20163 20.625 3.91548 20.5065 3.7045 20.2955C3.49353 20.0845 3.375 19.7984 3.375 19.5V4.5C3.375 3.8775 3.8775 3.375 4.5 3.375H19.5C20.121 3.375 20.625 3.8775 20.625 4.5V19.5C20.625 19.7984 20.5065 20.0845 20.2955 20.2955C20.0845 20.5065 19.7984 20.625 19.5 20.625ZM5.625 18.375H18.375V5.625H5.625V18.375Z"
													fill="#483183"/>
											</svg>
										)}
									</button>

...
export default function ClassCard({ courseNumber, courseName }: ClassCardProps){
    ...
    <div className="bg-[#EDEFFA] rounded-[10px] shadow-md p-4 flex flex-col justify-end h-[263px] w-[278px]">
        ...
        {/* class name + inbox symbol */}
        <h2 className="text-[20px] font-bold text-black flex items-center gap-[146px]">
            <span className="whitespace-nowrap">{courseNumber}</span>
            <button className="pointer-cursor" onClick={() => {toggleCheck(crs.id); toggleArchived(crs);}>
        ...
        <p className="text-[12px] font-normal text-black"><strong>UC Davis | </strong>{courseName}</p>
    </div>
    );
}